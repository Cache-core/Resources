...
Open the wallets with cache-serivce like so:
./cache-service -w walletA.wallet -p yourpass --rpc-password test --bind-port 8070
./cache-service -w walletB.wallet -p yourpass --rpc-password test --bind-port 8071
...
moveDecimal = 100000  # CXCHE has 5 decimals so 100000 is the divide/multiply factor
minAmount = 1 * moveDecimal  # min number for amount to xfer
maxAmount = 5000 * moveDecimal  # max number for amount to xfer
anonymity = 0
fee = 100 # atomic units, CXCHE would be 0.00100 as the tx network fee

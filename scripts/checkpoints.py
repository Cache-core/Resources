...
def height():
    base_url = 'http://51.79.100.195:40000/getheight'
    resp = requests.get(base_url).json()
    if 'height' not in resp:
        print('Unexpected response, make sure the cache-daemon is running',
               resp)
        sys.exit(-1)
    else:
        return resp['height']
...
def rpc(method, params={}):
    base_url = 'http://51.79.100.195:40000/json_rpc'
    payload = {
        'jsonrpc': '2.0',
        'id': 'block_info',
        'method': method,
    ...
    if 'result' not in resp:
        print('Unexpected response, make sure the cache-daemon is running' # with block explorer enabled'
               , resp)
        sys.exit(-1)
    else:
        return resp['result']
